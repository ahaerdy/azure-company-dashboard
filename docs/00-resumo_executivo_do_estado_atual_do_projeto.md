...
| 7 | Correção dos valores de `Hours` (divisão por 10 — separador decimal do CSV) | `works_on` |
...
- 5 relacionamentos configurados no Power BI.
- `works_on` definida como tabela fato central.
- Tabelas dimensão: `employee`, `departament`, `project`, `dependent`.
- Tabelas auxiliares sem relacionamento: `dept_locations`, `department_locations`, `employee_por_gerente`.
- Correção aplicada na coluna `Hours` da tabela `works_on` (valores divididos por 10 — problema de separador decimal na importação do CSV).
- Detalhes completos em: [05-modelo_dimensional.md](05-modelo_dimensional.md)

### 2.6 Verificação de Horas e Hierarquia ✅
...
1. **Criação de medidas DAX** para métricas executivas
2. **Desenvolvimento do dashboard corporativo**
3. **Publicação e documentação final**
